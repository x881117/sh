...
cd ~/ceremonyclient/node && git checkout release-cdn && screen -dmS quili bash -c "cd ~/ceremonyclient/node &&  rm -rf ./release_autorun.sh && wget -O https://raw.githubusercontent.com/x881117/sh/main/release_autorun.sh && chmod +x release_autorun.sh && ./release_autorun.sh'" && screen -r quili
...
# 独立启动
function run_node() {
   cd ~/ceremonyclient/node && git checkout release-cdn && ./release_autorun.sh'"
...
screen -dmS quili bash -c ' cd ~/ceremonyclient/node && git checkout release-cdn && ./release_autorun.sh'
